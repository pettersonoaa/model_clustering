...
N_SAMPLES = 10000 # Target number of samples
N_FEATURES = 20
N_CLUSTERS = 5 
CLUSTER_STD = 6  # Standard deviation of clusters
...
def fit_selected_model(model_name: str, X):
    scaler = StandardScaler()
    preprocessor = Pipeline([('scaler', scaler)])
    start_time = time.time()
    X_processed = preprocessor.fit_transform(X)
...
def find_best_k_elbow(X, max_k=10, show=False):
    """
    ...
    for k in range(2, max_k + 1):
        print(f"Fitting {k-1}/{max_k-1} model...")
        start_time = time.time()
    ...
    # Save the figure
    output_file_name = f"{OUTPUT_FILES}/elbow_method.png"
...
labels, data = fit_selected_model(model_name=SELECTED_MODEL, X=X)
if ELBOW:
    find_best_k_elbow(X=X, max_k=10, show=SHOW_PLT)

visualize_clustering_results(
    model_name=SELECTED_MODEL, 
    data_for_viz=data, 
    labels_for_viz=labels, 
    show=SHOW_PLT
)
